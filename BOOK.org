...
* Lisp-Predicate Negative Tests

This section provides comprehensive negative testing for all lisp-predicates defined in ε-prolog. While positive tests demonstrate correct behavior, negative tests ensure proper error handling and failure conditions, which is crucial for robust Prolog programs.

These tests verify that lisp-predicates correctly fail when they should, handle invalid inputs gracefully, and maintain proper logical semantics even in edge cases. Each lisp-predicate has dedicated negative test coverage to ensure comprehensive validation.
