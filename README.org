...
ε-prolog includes a comprehensive test suite in =USAGE.org= with 59 tests covering all major functionality, including examples and complete test coverage.

** Running Tests

*** Command Line (Recommended)
Run all tests from the command line using the Makefile:
...
#+BEGIN_SRC bash
emacs -batch -l eprolog.el -l USAGE.el --eval "(ert-run-tests-batch-and-exit \"eprolog-usage-\")"
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
;; Load the usage file (which contains all tests)
(org-babel-load-file "USAGE.org")

;; Run all tests interactively
(ert-run-tests-interactively "eprolog-usage-")

;; Run a specific test
(ert-run-tests-interactively "eprolog-usage-fibonacci")
