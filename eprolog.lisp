;;;; eprolog.lisp --- Native Prolog engine implementation in Common Lisp
...
;; in pure Common Lisp.  It provides a fully functional Prolog system
;; with traditional Prolog programming capabilities and seamless Lisp
;; interoperability.
;;
;; Features:
...
(define-condition eprolog-cut-exception (error)
  ...
  (:documentation "Exception used to implement cut semantics"))
...
;;; Helper function for proper list checking
...
(declaim (ftype (function (t t t) t) eprolog--unify))
(declaim (ftype (function (t t) t) eprolog--prove-goal))
(declaim (ftype (function (t t t) t) eprolog--search-matching-clauses))
(declaim (ftype (function (t t t) t) eprolog--with-spy))
(declaim (ftype (function (t &rest t) t) eprolog-solve))
...
Variables are deduplicated with the rightmost occurrence preserved."
  (labels ((collect-all-variables (tree)
             (if (atom tree)
                 (list tree)
               (append (collect-all-variables (car tree))
                       (collect-all-variables (cdr tree))))))
    (let* ((all-atoms (collect-all-variables expression))
           (variables-only (remove-if-not #'eprolog--named-variable-p all-atoms)))
      (remove-duplicates variables-only :from-end t))))
...
CLAUSES can also be a function for built-in predicates implemented in Lisp."
  (let* ((cleaned-db (remove predicate-symbol *eprolog-clause-database* :key #'car))
...
preserves the original variable's base name."
  (labels ((make-renaming-pair (variable)
    ...
    (let* ((variables (eprolog--variables-in expression))
      ...
      (sublis alist expression :key #'symbol-to-string :test #'string=))))
        ...
        (make-eprolog--success
         ...
         (lambda ()
           (handler-case
             ...
             (eprolog-cut-exception (err)
               (if (eq tag (exception-tag err))
...
(defun eprolog--printf (format-string &rest args)
  "Print formatted output using FORMAT-STRING and ARGS.
Wrapper around `format' and `princ' for consistent output handling
throughout the Prolog engine."
  (princ (apply #'format nil format-string args)))
...
(defun eprolog--y-or-n-p (prompt)
  "Simple yes-or-no prompt implementation for eprolog."
  (format t "~A (y or n) " prompt)
  (force-output)
  (let ((response (read)))
    (eq response 'y)))
      ...
      (eprolog-solve
       ...
       (lambda (solution)
         (let* ((formatted-solution
                ...
                (prompt (concatenate 'string formatted-solution (format nil "~%~%Continue?"))))
           (push solution solutions)
           (unless (eprolog--y-or-n-p prompt)
             (return-from query-exit solutions))))
       :failure
       (lambda ()
         (format t "~%No~%")
         (return-from query-exit solutions))))))
...
Examples:
  ...
  (eprolog-solve '((parent tom _x))
    :success (lambda (bindings) (format t \"Found: ~S~%\" bindings))
    :failure (lambda () (format t \"No more solutions~%\")))"
  (let* ((on-success (or (getf args :success) (lambda (x) (declare (ignore x)))))
         (on-failure (or (getf args :failure) (lambda ()))))
...
(defmacro eprolog-define-lisp-predicate (name args &body body)
...
(defmacro eprolog-define-prolog-predicate (head &body body)
...
(defmacro eprolog-define-predicate (head &body body)
  `(eprolog-define-prolog-predicate ,head ,@body))
...
for the current goal. CHOICE-POINT identifies the choice point to cut."
  (let ((continuation-result (eprolog--prove-goal-sequence *eprolog-remaining-goals* *eprolog-current-bindings*)))
    (error 'eprolog-cut-exception :tag choice-point :value continuation-result)))
...
(eprolog-define-lisp-predicate or (&rest goals)
  (let* ((or-2-goal (reduce (lambda (expr acc) `(or-2 ,expr ,acc)) goals :initial-value 'false))
  ...
  (if (null body)
    ...
    (let* ((element (car body))
      ...
      (cond
        ...
        ((and (vectorp element) (= (length element) 0))
        ...
        ((and (consp element) (not (eq (car element) '@)))
